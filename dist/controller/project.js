...
async function createProject(data, email) {
    ...
    try {
        ...
        const article = repo.save(new projects_1.Project(await slugify_1.slugify(data.title), data.title, data.links, data.body, data.taglist, await sanitization_1.sanitization(user)));
        return article;
    }
    catch (e) {
        throw e;
